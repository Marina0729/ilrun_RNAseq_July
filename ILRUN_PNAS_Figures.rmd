...
Figure2C <- ggplot(DEgene_validation, aes(y= log2FoldChange, x=GENE_ID, fill = direction))+
  geom_bar(stat= "Identity", color= "black", position = position_dodge()) +
  facet_wrap(~ assay, dir = "v")+
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
  geom_text(aes(label = padj_up),position=position_dodge(width=0.9), vjust=-3.7, size = 3)+
  geom_text(aes(label = padj_down),position=position_dodge(width=0.9), vjust=2.7, size = 3)+
  ylim(-5, 7)+
