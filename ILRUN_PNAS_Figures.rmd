...
Figure2C <- ggplot(DEgene_validation, aes(y= log2FoldChange, x=GENE_ID, fill = direction))+
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
  geom_text(aes(label = padj_up),position=position_dodge(width=0.9), vjust=-2.8, size = 5)+
  geom_text(aes(label = padj_down),position=position_dodge(width=0.9), vjust=2.7, size = 5)+
  ylim(-5, 7)+
...
Figure3C_violin <-ggplot() +
  ...
  geom_point(data=ACE2_TCID50_stats, aes(x=ng_ACE2, y=mean), size=2, color = "red") +
