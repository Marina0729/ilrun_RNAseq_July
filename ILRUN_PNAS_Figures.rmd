...
Figure2C <- ggplot(DEgene_validation, aes(y= log2FoldChange, x=GENE_ID, fill = direction))+
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
  geom_text(aes(label = padj_up),position=position_dodge(width=0.9), vjust=-1.8, size = 5)+
  geom_text(aes(label = padj_down),position=position_dodge(width=0.9), vjust=2.0, size = 5)+
  ylim(-3, 3)+
