...
polyIC_reps_IFNb <- read.csv("data/Caco2_polyIC_reps.csv") %>%
  filter(treatment == "polyIC") %>% 
  filter(target == "IFNb")

polyIC_reps_TNFa <- read.csv("data/Caco2_polyIC_reps.csv") %>%
  filter(treatment == "polyIC") %>% 
  filter(target == "TNFa")
...
polyIC_aov_TNFa <- aov(relative_mRNA ~ siRNA, data = polyIC_reps_TNFa)
...
polyIC$treatment <- factor(polyIC$treatment,levels =c("control", "polyIC"))
...
##### plot the data ################
 IFNb <- polyIC %>% 
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
  geom_signif(comparisons=list(c("siNEG", "siILRUN")), annotations="***", y_position = 1400, tip_length = 0, vjust=0.4) +
 ...
 TNFa <- polyIC %>% 
  ...
  geom_bar(stat= "Identity", color= "black", position = position_dodge()) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
   geom_signif(comparisons=list(c("siNEG", "siILRUN")), annotations="**", y_position = 35, tip_length = 0, vjust=0.4) +
  scale_fill_manual(values = c("#56B4E9", "#0072B2"))+
  theme_set(theme_classic(base_size = 8)) +
  labs(title = "TNFa",
       y ="",
  ...
  geom_bar(stat= "Identity", color= "black", position = position_dodge()) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width =0.2, position = position_dodge(0.9)) +
    geom_signif(comparisons=list(c("siNEG", "siILRUN")), annotations="***", y_position = 28, tip_length = 0, vjust=0.4) +
  scale_fill_manual(values = c("#56B4E9", "#0072B2"))+
  theme_set(theme_classic(base_size = 8)) +
  labs(title = "IL6",
       y ="",
...
plot_cytokines <- plot_grid(IFNb + theme(legend.position="none"),
                  TNFa + theme(legend.position="none"),
...
```{r validation of IL6R, VIM and ISG15}

```


```{r Figure3A. Enriched pathways }
...
###### Pathway enrichment  ##############
Figure3A <- ggplot(pathway_ILRUNgenesUninf24hr) + 
...
cpm_infected$timepoint <- factor(cpm_infected$timepoint,levels =c("6hr", "24hr"))
